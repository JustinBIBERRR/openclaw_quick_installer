...
本版本重点优化了安装可见性、首次配置链路和失败恢复体验。

- 安装阶段改为可见 PowerShell 执行，`npm install -g openclaw@latest` 输出可直接被用户观察。
- 安装前新增 `openclaw --version` 预检，已安装则跳过重复安装并回传 UI 提示。
- 新增“可选能力配置（onboard）”步骤，支持 daemon/channel/skills/hooks/启动方式按能力 best-effort 执行且可跳过。
- API 配置页升级为 `Provider -> Model -> API Key` 结构，支持读取已有配置并自动预填。
- Gateway 启动前增加认证与默认模型自愈补齐，减少 `No API key found` 与默认模型错误。
- Dashboard 打开逻辑优先 `openclaw dashboard`，失败时再回退本地 URL，避免 token mismatch。
- 清理调试埋点，保留正式日志与结构化错误输出。
