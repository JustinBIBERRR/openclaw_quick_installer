...
flowchart TD
  ...
  apiKey --> launch[Launching 启动 Gateway]
...
- `openclaw dashboard` 能力探测与后续接入预留
- `openclaw onboarding` 能力探测与后续接入预留
...
由 `Launching.tsx` 调用 Rust 的 `start_gateway`。

特点：

- Rust 直接拉起 `openclaw gateway`
- 日志通过事件流实时返回前端
- 启动完成后进入 Manager

### 路径 B：Manager 页后台启动

由 `Manager.tsx` 调用 `start_gateway_bg`，再由 `gateway.ps1` 在后台管理进程。

特点：

- 适合安装完成后的管理场景
- 落盘 `gateway-stdout.log` 与 `gateway-stderr.log`
- 支持后台状态轮询

维护者必须知道的事实是：

- 当前仓库仍有“前台 Rust 启动”和“后台 PowerShell 启动”两条并存路径
- 这是一层兼容性历史包袱，不是最终理想形态
- 未来应进一步统一到“官方命令优先 + 单一路径编排”

README 把这件事写出来，不是暴露问题，而是防止维护者误判系统边界。
